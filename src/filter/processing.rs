...
pub fn invert(lin: &ArrayView3<f32>, film_base: [f32; 3]) -> Array3<f32> {
    let shape = lin.raw_dim();
    let mut inv = Array3::<f32>::zeros(shape);

    for ch in 0..3 {
        let channel_film_base = film_base[ch];
...
pub fn invert_color_negative(negative_image: &ArrayView3<f32>, film_base: [f32;3]) -> Array3<f32> {
    let linear_negative_image: Array3<f32> = srgb_to_linear(&negative_image.view());
    let linear_positive_image: Array3<f32> = invert(&linear_negative_image.view(), film_base);
